...
docker exec -ti graph-deployer-prod sh -c "yarn graph-command deploy float-capital/float-capital-alpha${GRAPH_POSTFIX} --ipfs http://ipfs-prod:5001 --node http://graph-node-prod:8020 ./subgraph.polygon.yaml"
