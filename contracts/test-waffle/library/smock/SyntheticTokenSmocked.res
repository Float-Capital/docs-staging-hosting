...
@module("@eth-optimism/smock") external make: SyntheticToken.t => Js.Promise.t<t> = "smockit"
...
let mockIsLongToReturn: (t, bool) => unit = (_r, _param0) => {
  let _ = %raw("_r.smocked.isLong.will.return.with([_param0])")
}

type isLongCall

let isLongCalls: t => array<isLongCall> = _r => {
  let array = %raw("_r.smocked.isLong.calls")
  array->Array.map(() => {
    ()->Obj.magic
  })
}
...
let mockMarketIndexToReturn: (t, int) => unit = (_r, _param0) => {
  let _ = %raw("_r.smocked.marketIndex.will.return.with([_param0])")
}

type marketIndexCall

let marketIndexCalls: t => array<marketIndexCall> = _r => {
  let array = %raw("_r.smocked.marketIndex.calls")
  array->Array.map(() => {
    ()->Obj.magic
  })
}
