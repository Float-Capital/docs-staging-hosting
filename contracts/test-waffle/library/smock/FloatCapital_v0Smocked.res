...
let make = () => makeRaw("FloatCapital_v0")
...
@send @scope(("to", "have", "been"))
external initializeCalledWith: (expectation, Ethers.ethAddress) => unit = "calledWith"

@get external initializeFunction: t => string = "initialize"

let initializeCallCheck = (contract, {admin}: initializeCall) => {
  expect(contract->initializeFunction)->initializeCalledWith(admin)
}
