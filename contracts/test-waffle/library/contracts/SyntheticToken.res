...
let make: (~name: string,~symbol: string,~longShort: Ethers.ethAddress,~staker: Ethers.ethAddress,~marketIndex: int,~isLong: bool,) => JsPromise.t<t> = (~name,~symbol,~longShort,~staker,~marketIndex,~isLong,) =>
    ...
    let makeSmock: (~name: string,~symbol: string,~longShort: Ethers.ethAddress,~staker: Ethers.ethAddress,~marketIndex: int,~isLong: bool,) => JsPromise.t<t> = (~name,~symbol,~longShort,~staker,~marketIndex,~isLong,) =>
    deployMockContract6(contractName, name,symbol,longShort,staker,marketIndex,isLong,)->Obj.magic
