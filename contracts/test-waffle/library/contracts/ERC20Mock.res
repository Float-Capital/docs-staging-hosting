...
let make: (~name: string,~symbol: string,) => JsPromise.t<t> = (~name,~symbol,) =>
    ...
    let makeSmock: (~name: string,~symbol: string,) => JsPromise.t<t> = (~name,~symbol,) =>
    deployMockContract2(contractName, name,symbol,)->Obj.magic
  ...
  @send
  external setShouldMockTransfer: (
    t,~value: bool,
  ) => JsPromise.t<transaction> = "setShouldMockTransfer"
