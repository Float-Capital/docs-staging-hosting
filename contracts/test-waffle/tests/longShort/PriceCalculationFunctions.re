...
let testUnit =
  ...
  describe("_getSyntheticTokenPrice", () => {
    it("should test function returns correct price", () => { 
      ...
      let expectedResult = randomAmountPaymentToken->mul(CONSTANTS.tenToThe18)->div(randomAmountSynthToken);
  ...
  describe("_getAmountPaymentToken", () => {
    it("should test function returns correct amount", () => { 
      ...
      let expectedResult = randomAmountSynthToken->mul(randomTokenPrice)->div(CONSTANTS.tenToThe18);
      Chai.bnEqual(~message="expected result different to actual result for _getAmountPaymentToken call", actualResult, expectedResult); 
    }) 
  });
  ...
  describe("_getAmountSynthToken", () => {
    it("should test function returns correct amount", () => { 
      ...
      let expectedResult = randomAmountPaymentToken->mul(CONSTANTS.tenToThe18)->div(randomTokenPrice);
      Chai.bnEqual(~message="expected result different to actual result for _getAmountSynthTokencd call", actualResult, expectedResult);  
    }) 
  });
};
