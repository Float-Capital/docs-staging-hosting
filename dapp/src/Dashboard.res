...
module Header = {
  @react.component
  let make = (~children) =>                 
  <h1 className="font-bold text-center pt-5 text-lg font-alphbeta">
    {children}
  </h1>
}
  ...
  <AccessControl
    ...
    <div className="w-screen absolute flex flex-col left-0 top-0 mt-20 overflow-x-hidden">
      {<>
        ...
        let totalSynthValue = 
          DashboardCalcs.getTotalSynthValue(~totalValueLocked, ~totalValueStaked)
        ...
        <div className="min-w-3/4 max-w-full flex flex-col self-center items-center justify-start">
          ...
          <div className={"w-full flex flex-col md:flex-row justify-between mt-1"}>
            <Divider>
            {
              floatProtocolCard(~totalTxs, ~totalUsers, ~totalGasUsed)
            }
            </Divider>
            <Divider>
              {
