...
let make = () => {
  ...
  let (haveLocalSignInDetails, setHaveLocalSignInDetails) = React.useState(() =>
    Client.getAuthHeaders(~user=Some(userAddress)) != None
  )
  ...
  React.useEffect1(() => {
    ...
    | {data: Some({update_user: Some({returning: _})})} =>
      let _ = fetchUserDetails()
    | _ => ()
    }
    None
  ...
  let form = ChangeUsernameForm.useForm(~initialInput={username: ""}, ~onSubmit=(
    {username},
    _form,
  ) => {
    let _ = updateUsernameMutate(
      ~context=Client.createContext(Client.DB),
      {
  ...
  <>
    ...
    | ({data: Some({user: userArray})}, true) if userArray->Array.length > 0 =>
      <div>
        ...
        | Some({userName: Some(u)}) => <div> {`Your username is ${u}`->React.string} </div>
      ...
      <button
        ...
        onClick={_ => {
          ...
          let _ = Ethers.Wallet.signMessage(
            ...
            Client.setSignInData(
              ~ethAddress=userAddress->ethAdrToLowerStr,
