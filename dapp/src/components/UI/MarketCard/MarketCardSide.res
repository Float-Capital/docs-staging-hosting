...
let make = (~marketName, ~isLong, ~value, ~beta) => {
    ...
    <div className="w-1/4 flex items-center flex-grow text-sm flex-col">
        ...
        <div className="flex flex-col items-center justify-center pt-0 mt-auto">
            ...
            <span className="font-bold">{isLong ? "LONG"->React.string : "SHORT"->React.string  }
        ...
        <div className="text-sm text-center m-auto">
            <span className="font-bold">
              {`Exposure`->React.string}
            </span>
            <Tooltip tip={`The impact ${marketName} price movements have on ${isLong ? "long" : "short"} value`}/>
            <span className="font-bold">
              {`: `->React.string}
            </span>
            {`${~beta}%`->React.string}
