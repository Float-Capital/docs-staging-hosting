...
let make=(~percentStrLong, ~percentStrShort) => {
    <div className="relative w-full h-6 my-1">
            ...
            <div style={ReactDOM.Style.make(~width=`${percentStrLong}%`, ())} className="h-8 bg-blue-400"/>
            <div style={ReactDOM.Style.make(~width=`${percentStrShort}%`, ())} className="h-8 bg-blue-300"/>
            </div>
    </div>
}