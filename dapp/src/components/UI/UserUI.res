module UserContainer = {
  @react.component
  let make = (~children) => {
    <div className=`min-w-3/4 flex flex-col items-center`>
      {children}
    </div>
  }
}
  ...
  let make = () => {
    <div className=`p-5 mt-7 flex bg-white bg-opacity-75 rounded-lg shadow-lg`>
      <span className=`text-sm`>
        {`💲 BUSD Balance: $1234.todo`->React.string}
      </span>
...
module UserColumnCard = {
  @react.component
  let make = (~children) => {
    <div className=`bg-white w-full bg-opacity-75 rounded-lg shadow-lg p-4`>
      {children}
    </div>
  }
}
...
module UserProfileHeader = {
  ...
  let make = (~name, ~level) => {
    <div className="w-full flex flex-row justify-around">
      <div className="w-24 h-24 rounded-full border-2 border-light-purple flex items-center justify-center">
        <img className="inline h-10" src="/img/mario.png" />
      </div>

      <div className="flex flex-col text-center justify-center">
        <div>{name->React.string}</div>
        <div>{`Lvl. ${level}`->React.string}</div>
      </div>
    </div>
  }
}
...
module UserMarketBox = {
  ...
  let make = (~name, ~isLong, ~tokens, ~value, ~children) => {
    <div className=`flex w-11/12 mx-auto p-2 mb-2 border-2 border-light-purple rounded-lg z-10 shadow`>
      ...
      <div className=`w-1/3 text-sm mx-2 text-center self-center`>
        ...
        <span className=`text-xs`>
          {Js.String.concat(`${value}`, `~$`)->React.string}
        </span>
      </div>
...
module UserMarketUnstake ={
  ...
  let make = () => {
    <div className=`flex flex-col`>
      <span className="text-xxs self-center">
        <i>{`4 days ago`->React.string}</i>
      </span>
      <Button variant="tiny" onClick={_ => ()}>{`redeem`}</Button>
    </div>
  }
