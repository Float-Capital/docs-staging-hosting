...
eg. ellipsifyMiddle(~inputString="abcdefghijk", ~maxLength=6, ~trailingCharacters=3) == "abc...ijk"
`)
let ellipsifyMiddle = (~inputString, ~maxLength, ~trailingCharacters) => {
...
@react.component
let make = (~address) => {
  ellipsifyMiddle(~inputString=address, ~maxLength=8, ~trailingCharacters=3)->React.string
}
