...
@react.component
let make = (~orderPostion, ~orderPostionMobile, ~marketName, ~isLong, ~apy, ~floatApy, ~beta, ~stakeApy) => {
  let _s = stakeApy
  ...
  <div className="flex flex-col justify-center pt-0 my-1">
    ...
    <p className="text-sm tracking-widest font-alphbeta"> {`${(apy *. 100.)->Js.Float.toFixedWithPrecision(~digits=2)}%`->React.string} </p>
  </div>
}
  ...
  <div
    ...
    {switch(apy, stakeApy){
      | (Loaded(apyVal), Loaded(stakeApy)) => {
          ...
          <>
          {ifElement(apyGreaterThanZero, apyComponent(~header="SYNTH", ~apy=apyVal, ~suffix=React.null))}
          {ifElement(stakeApyGreaterThanZero && apyGreaterThanZero, <span className="mx-1"> {"+"->React.string} </span>)}
           {ifElement(stakeApyGreaterThanZero, apyComponent(~header="STAKE", ~apy=stakeApy, ~suffix=<span className="ml-1"><Tooltip tip="Expected yield from FLOAT buybacks"/></span>))}
          </>
      }
      | _ => <Loader.Tiny />
    }}
